...
export default async function checkUserInput(              
        ...
        const subjectEndMessage = (() => {
            ...
            return isLastSubject 
                ? "4.ユーザへの返答を生成しなさい。ただし面接はこれで終了なので簡単な相槌と、面接を終了する旨を伝えるのみにしてください。質問リストは既に用意してあるので、あなたは次の質問を考えなくていいです" 
                : "4.ユーザへの返答を生成しなさい。ただしこの話題はこれ終了なので簡単な相槌と、次の話題に移る旨を伝えるのみにしなさい。質問リストは既に用意してあるので、あなたは次の質問を考えなくていいです。";
            } else {
            return isLastSubject 
                ? "4.ユーザへの返答を生成しなさい。ただしisSubjectEndがtrueの場合は「簡単な相槌と、面接を終了する旨を伝えるのみ」falseの場合は「相槌と、深堀質問を1つだけ生成する」にしなさい。"
                : "4.ユーザへの返答を生成しなさい。ただしisSubjectEndがtrueの場合は「簡単な相槌と、次の質問に移る旨を伝えるのみ」falseの場合は「相槌と、深堀質問を1つだけ生成する」にしなさい。";
            }
        })();
